...
function(resp, pred, mult, nb, maxIter=200) {
  ...
  for(run in 1:maxIter){
	...
	##derivative
	newTheta <- theta.optim(resp, pred, mult, ini.Theta, curr.X0)
    ...
    like.theta1 <- gsmult.mrf.log.likelihood(resp = resp, coeff = coeff1, pred = pred, sigma = sigma1, w = mult, beta = beta1)
