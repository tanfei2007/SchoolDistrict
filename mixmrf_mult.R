...
function(resp, pred, mult, nb, maxIter=200) {
  ...
  for(run in 1:maxIter){
	...
	##derivative
	newTheta <- mult.theta.optim(resp, pred, mult, ini.Theta, curr.X0)
    ...
    like.theta0 <- mult.mrf.log.likelihood(resp = resp, coeff = coeff0, pred = pred, sigma = sigma0, w = mult, beta = beta0)
    ...
    like.theta1 <- mult.mrf.log.likelihood(resp = resp, coeff = coeff1, pred = pred, sigma = sigma1, w = mult, beta = beta1)
